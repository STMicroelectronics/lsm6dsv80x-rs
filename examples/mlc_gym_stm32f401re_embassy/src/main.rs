...
async fn main(_spawner: Spawner) {
    ...
    loop {
        ...
        if status.mlc1 == 1 {
            let mlc_out = sensor.mlc_out_get().unwrap();
            match mlc_out.mlc1_src {
                4 => {
                    writeln!(&mut msg, "biceps curl event").unwrap();
                }
                8 => {
                    writeln!(&mut msg, "Lateral raises event").unwrap();
                }
                12 => {
                    writeln!(&mut msg, "Squats event").unwrap();
                }
                _ => {}
            }
            let _ = tx.blocking_write(msg.as_bytes());
