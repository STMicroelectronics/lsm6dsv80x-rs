# LSM6DSV80X Sensor Data Acquisition on STM32F401RE with IKS4A1 Shield (JSON-Configured FSM Events)
...
- The sensor is configured by applying a sequence of register writes and delays generated by the **`ucf-tool`** from a JSON file (`lsm6dsv80x_four_d.ucf`), which programs the sensor's FSM for gesture detection.
